...
def create_csv():
    ...
    csv.to_csv(f"home/buga/wetter-screen/Data/{datetime.today().date()}_Temperatures.csv", sep=";", index = False)
...
def import_values_to_csv(time, temperatureOutside, temperatureCorridor, above30):
    dataframe = pd.read_csv(f"home/buga/wetter-screen/Data/{datetime.today().date()}_Temperatures.csv", sep = ";")
    ...
    dataframe.to_csv(f"home/buga/wetter-screen/Data/{datetime.today().date()}_Temperatures.csv",  sep=";", index = False)
